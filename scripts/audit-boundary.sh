...
run_check 'monitor logical box access outside compat' '->logicalBox\(\)'
run_check 'window main surface box access outside compat' '->getWindowMainSurfaceBox\(\)'
run_check 'window workspace id access outside compat' '->workspaceID\(\)'
run_check 'workspace monitor id access outside compat' '->monitorID\(\)'
run_check 'config value access outside compat' 'CConfigValue<'
run_check 'drift-prone Hyprland implementation includes outside compat' '#include <hyprland/src/(config/ConfigManager|config/ConfigValue|layout/LayoutManager|managers/KeybindManager|managers/PointerManager|managers/animation/DesktopAnimationManager|managers/input/InputManager)\.hpp>'
